...
| `pdf/contracte-lloguer.pdf` (2 pages) | A rental contract | Catalan | Signed by both parties. Rent 1.150 €/month, 1-year term. Unusual or likely abusive clauses to flag: **3 months' deposit**, **agency fees paid by the tenant**, **tenant pays IBI**, **tenant pays all repairs**, **all remaining rent as the early-exit penalty**, **landlord enters at any time without notice**. Should suggest the Oficina d'Habitatge. |
| `pdf/contracte-lloguer-sense-signar.pdf` (2 pages) | A rental contract | Catalan | Same contract, fully filled in but **not signed**: both signature boxes and the place/date of signing are empty. Claude should say it isn't binding yet, so this is the moment to negotiate the abusive clauses before signing. |
| `pdf/contracte-lloguer-vencut.pdf` (2 pages) | A rental contract | Catalan | Same contract (rent 950 €), signed 1 Sep 2020 for 5 years: it **ended on 30 Sep 2025**, a year before the demo date. Claude should notice the end date has passed, explain that it may have been extended automatically if nobody gave notice, and suggest confirming with the landlord or the Oficina d'Habitatge before relying on it. |
| `pdf/contracte-lloguer-sense-dades.pdf` (2 pages) | A rental contract | Catalan | Same contract, but the **tenant's name, DNI/NIE and address are blank** and the tenant's signature line has no name. Claude should say it can't tell who the tenant is, warn against signing (or letting anyone sign) with blanks, and still flag the abusive clauses. |
